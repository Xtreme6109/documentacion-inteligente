...
namespace DocumentacionInteligente.BackEnd.Controllers
{
    ...
    public class ReporteController : ControllerBase
    {
        ...
        [HttpGet("TokensPorUsuario")]
        public IActionResult GetTokensPorUsuario()
        {
            var tokensPorUsuario = _context.HISTORIALDOCUMENTOSIA
                .GroupBy(h => h.USUARIO_ID)
                .Select(g => new
                {
                    usuarioId = g.Key,
                    nombreUsuario = _context.USUARIOS
                        ...
                        .Select(u => u.NOMBRE)
                        .FirstOrDefault(),
                    tokensTotales = g.Sum(x => x.TOKENS_ENTRADA + x.TOKENS_SALIDA)
                })
                .ToList();
